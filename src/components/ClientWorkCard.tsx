...
export const ClientWorkCard = ({ work, onClick }: ClientWorkCardProps) => {
  return (
    <Card 
      ...
      onClick={onClick}
    >
      <CardHeader className="p-4 md:p-6 flex-1 flex flex-col relative">
        <div className="w-full h-40 bg-gray-200 rounded-lg mb-4 overflow-hidden flex-shrink-0">
          <FlexibleImage
            source={work.image}
        ...
        <div className="flex-1 flex flex-col min-h-0 pb-12">
          <CardTitle className="font-manrope text-lg md:text-xl text-gray-900 mb-2 line-clamp-2">
            {work.title}
          </CardTitle>
          <div className="text-[#F4A42C] font-manrope font-semibold mb-3 text-sm md:text-base">
            {work.scope}
          </div>
          <CardDescription className="text-gray-600 font-manrope leading-relaxed text-sm md:text-base flex-1 overflow-hidden">
        ...
        <div className="absolute bottom-4 right-4 flex items-center text-[#EA3E3A] font-manrope font-semibold text-sm whitespace-nowrap">
